...
import com.cookccook.exceptions.FileNotExistsException;
...
public class FileHandler {
	...
	public File getStoredRcpFile(String fileName) {
		return new File(baseDir+"/recipeImages/", fileName);
	}
	...
	public StoredFile storeFile(File file) {
		// 사용자가 파일을 업로드 하지 않았다면 null을 반환한다.
			if (file == null || !file.exists() || file.isDirectory()) {
				...
				// 파일이 저장될 위치를 지정한다.
				File storePath = new File(baseDir+"/recipeImages/", fileName);
				...
				try {
					...
					Files.copy(file.toPath(), storePath.toPath(), copyOptions);
...
public StoredFile storeRcpTitleImgFile(MultipartFile multipartFile) {
