...
<script type="text/javascript">
	$().ready(function() {
		...
		$(".refreshBtn").on('click', function(){
			location.href="/seller/orderlist"
		})

		//새창(상품수정) 열고 닫기
		$(".popupBtn").on('click', function(){
			let purchaseProductId = $(this).children(".purchaseProductId").text()
			let url = "/seller/orderdetail/" + purchaseProductId
			let name = "주문상세페이지"
			var options = 'top=600, left=500, width=700, height=600, status=no, menubar=no, toolbar=no, resizable=no';
...
<jsp:include page="../layout/header_seller.jsp"></jsp:include>
	...
	<form:form modelAttribute="purchaseSearchVO" method="post" enctype="multipart/form-data">
		<div class="search">
			<div>
				<label for="purState">주문상태</label>
				<select id="purState" name="purState" value='${purchaseSearchVO.purState}'>
	...
	<div>
		...
		<p>총 ${purchaseProductCnt}건의 게시글이 검색되었습니다.</p>
		<c:choose>
			<c:when test="${not empty purchaseProductList }">
				<table>
					<tr>
						<th>주문번호</th>
						<th>주문상태</th>
					...
					<c:forEach items="${purchaseProductList }" var="purchaseProductVO">
					<tr> 
						<td class="number popupBtn">
							<p class="purchaseProductId" style="display: none;">${purchaseProductVO.purchaseProductId}</p>
							${purchaseProductVO.purchaseVO.purchaseId}</td> <!--<a href="/seller/productmodify/${productVO.productId}"></a>-->
						<td class="title">
							<c:if test="${purchaseProductVO.purState == 1}">결제완료</c:if>
							<c:if test="${purchaseProductVO.purState == 2}">상품준비중</c:if>
							<c:if test="${purchaseProductVO.purState == 3}">배송중</c:if>	
							<c:if test="${purchaseProductVO.purState == 4}">배송완료</c:if>	
						</td>
						...
						<td class="deliverFee">${purchaseProductVO.deliverFee}원</td>
